...
Cypress.Commands.add("movieCreate", () => {
    cy.request({
        ...
        body: {
            ...
            description: " é um filme estadunidense de 1984, do gênero ficção científica, dirigido por David Lynch, com roteiro baseado no romance homônimo de Frank Herbert.",
...
Cypress.Commands.add("userLogIn", () => {
    ...
    cy.userMock().then((createdUser) => {
        cy.request("POST", "https://raromdb-3c39614e42d4.herokuapp.com/api/users", createdUser)
            .then(({ body }) => {
                Cypress.env("actualUser", body);

                return cy.request("POST", "https://raromdb-3c39614e42d4.herokuapp.com/api/auth/login", {
